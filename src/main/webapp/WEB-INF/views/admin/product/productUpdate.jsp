...
<p><br></p>
	<div class="container-lg">
		...
		<form name="myform" method="post" enctype="multipart/form-data">
			<div class="form-group">
				...
				<select id="categoryMainCode" name="categoryMainCode" class="form-control" onchange="categoryMainChange()">
					<c:forEach var="mainVo" items="${categoryMainVos}">
						<option value="${mainVo.categoryMainCode}" ${mainVo.categoryMainCode == categoryMain ? 'selected' : ''}>${mainVo.categoryMainName}</option>
					</c:forEach>
				</select>
			</div>
			<div class="form-group">
				...
				<select id="categoryMiddleCode" name="categoryMiddleCode" class="form-control">
					<option value="">중분류</option>
					 <c:forEach var="middleVo" items="${categoryMiddleVos}">
						<option value="${middleVo.categoryMiddleCode}" ${middleVo.categoryMiddleCode == categoryMiddle ? 'selected' : ''}>${middleVo.categoryMiddleName}</option>
					</c:forEach> 
				</select>
			</div>
			<div class="form-group">
        <label for="productName">상품(모델)명</label>
        <input type="text" name="productName" id="productName" class="form-control" value="${vo.productName}" required/>
      </div>
			<div class="form-group">
        <label for="file">현재 썸네일</label>
        <img src="${ctp}/product/${vo.FSName}" width="150px" height="150px">
      ...
      <div class="form-group">
      	...
      	<input type="text" name="mainPrice" id="mainPrice" class="form-control" value="${vo.mainPrice}" required/>
      </div>
      <div class="form-group">
      	<label for="detail">상품 기본설명</label>
      	<input type="text" name="detail" id="detail" class="form-control" value="${vo.detail}" required/>
      </div>
      <div class="form-group">
