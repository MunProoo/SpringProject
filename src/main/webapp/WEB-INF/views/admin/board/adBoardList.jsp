...
$(function($){
  ...
  // 전체선택
	...
	// 게시글 선택 삭제
	$("input[name=deleteBoardList]").click(function(){
		var ans = confirm("선택한 게시글들을 삭제하시겠습니까?");
    if(!ans) return false;
      ...
      idxArr += el.value + '/';
		...
		if(idxArr == ""){
			alert("삭제하실 게시글을 선택하세요");
		}
		else{
			$.ajax({
				type: "post",
				url: "${ctp}/admin/deleteBoardList",
				data : {
					idxArr : idxArr
				},
				success : function(){
...
function pageCheck() {
 	...
 	location.href = "${ctp}/admin/adBoardList?pag=${pag}&pageSize="+pageSize;
 }
 ...
 function latelyCheck() {
 	...
 	if(lately == "") {
 		...
 		location.href="${ctp}/admin/adBoardList?page=${pag}&pageSize=${pageSize}&lately="+lately;
 	}
 }
...
<p><br></p>
    ...
    <tr>
      <td colspan="2" class="p-0"><h2>자유 게시판 관리</h2></td>
    </tr>
    <tr>
  ...
  <table class="table table-hover">
    <tr class="table-dark text-dark text-center">
      ...
      <th>조회수</th>
      <th>좋아요</th>
      <th style="width:100px"><input type="button" class="btn btn-warning" id="deleteBoardList" name="deleteBoardList" value="선택 모두 삭제" style="padding:5px;margin:10px;" /></th>
    </tr>
	    ...
	    <tr style="text-align: center">
	      ...
	      <td>
	        <c:if test="${vo.imgCheck == 'ON'}" >
		        <a href="${ctp}/board/boardContent?idx=${vo.idx}&pag=${pag}&pageSize=${pageSize}&lately=${lately}">${vo.title} <i class="far fa-image"></i></a>
	        </c:if>
	        <c:if test="${vo.imgCheck != 'ON'}" >
	      ...
	      <td>${vo.readNum}</td>
	      <td>${vo.good}</td>
	      <td><button type="button" class="btn btn-secondary btn-sm btnFont" onClick="deleteBoard(${vo.idx})">X</button></td>
	    </tr>
...
<div class="container text-center">
  <form name="searchForm" method="post" action="${ctp}/board/adBoardSearch">
    <b>검색 : </b>
