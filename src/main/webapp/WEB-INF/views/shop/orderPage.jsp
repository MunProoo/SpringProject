		  ...
		  <c:forEach var="vo" items="${orderVos}">  <!-- 세션에 담아놓은 orderVos의 품목내역들을 화면에 각각 보여주는 작업처리 -->
		    ...
		    <tr align="center">
		      ...
		      <td align="left">
		        ...
		        <p><br/>모델명 : <span style="color:orange;font-weight:bold;">${vo.productName}</span><br/>기본가격 : &nbsp;<fmt:formatNumber value="${vo.mainPrice}"/>원</p><br/>
		        <c:set var="optionNames" value="${fn:split(vo.optionName,',')}"/>
		        <c:set var="optionPrices" value="${fn:split(vo.optionPrice,',')}"/>
		        <c:set var="optionNums" value="${fn:split(vo.optionNum,',')}"/>
		        <p>
		          - 주문 옵션 종류 : 총 ${fn:length(optionNames)}개<br/>
		          <c:forEach var="i" begin="1" end="${fn:length(optionNames)}">
		            &nbsp; &nbsp; ㆍ ${optionNames[i-1]} - ${optionNums[i-1]}개 : <fmt:formatNumber value="${optionPrices[i-1]}"/>원<br/>
		          </c:forEach> 
		        </p>
		      </td>
		      <td>
