...
//		return "redirect:/product/orderPage";
	...
	public String cartListPost(String mid, int orderTotalPrice, OrderVO vo, Model model, HttpServletRequest request, HttpSession session) {
		...
		for(String idx : idxChecked) { // 체크한만큼 카트 담자
			...
			int quantity = Integer.parseInt(request.getParameter("quantity" + idx));
			...
			// 장바구니 물품들에 수량 조정처리
			// 옵션들 수량 * quantity
			String optionNum[] = cartVo.getOptionNum().split(",");
			String finalOptionNum = "";
			for(int i=0; i<optionNum.length; i++) {
				optionNum[i] =  String.valueOf((Integer.parseInt(optionNum[i]) * quantity));
				finalOptionNum += optionNum[i]+",";
			}
			finalOptionNum = finalOptionNum.substring(0, finalOptionNum.lastIndexOf(","));
			orderVo.setOptionNum(finalOptionNum);

			// 옵션별 가격 * quantity 
			String optionPrice[] = cartVo.getOptionPrice().split(",");
			String finalOptionPrice = "";
			
			for(int i=0; i<optionPrice.length; i++) {
				optionPrice[i] =  String.valueOf((Integer.parseInt(optionPrice[i]) * quantity));
				finalOptionPrice += optionPrice[i]+",";
			}
			finalOptionPrice = finalOptionPrice.substring(0, finalOptionPrice.lastIndexOf(","));
			orderVo.setOptionPrice(finalOptionPrice);
			
			// 최종가격 * quantity
			orderVo.setTotalPrice(cartVo.getTotalPrice()*quantity);
